...
An [estimated 2 trillion photos](http://ben-evans.com/benedictevans/2015/8/19/how-many-pictures) were shared online in 2015, with that number expected to continue to grow.  Lacking efficient methods to examine and identify objects in these images they can only be understood by the text that is used to summarize or tage them not the actual image content.

Being able to identify different aspects of an image would provide considerable value to a brand seeking to connect with its users.  This type of structured data is already [available for text](https://www.diffbot.com/) but what about images?  There is significant value in a brand or company being able to understand when and where it's products and services are being shown in images online.
...
Can you [teach a computer](http://www.ted.com/talks/fei_fei_li_how_we_re_teaching_computers_to_understand_pictures) to recognize  the brand logos of Nike and Altra in an unlabeled feed of images from Instagram?
...
This project solves a classification problem.  In its first incarnation discerning between 2 brands represents a _supervised binary classification_ problem.  In this case, selecting either [Nike](http://www.nike.com/) or [Altra](https://www.altrarunning.com/) products are present.

![NikeLogo](NikeLogo.png)
![AltraLogo](AltraLogo.png)
...
##### Cross-Entropy
Cross-entropy is a measure of a model's loss or cost.  A high value represents a model doing a poor job.  We seek to minimize the cross-entropy.   A more in-depth discussion of cross-entropy is [available here](http://neuralnetworksanddeeplearning.com/chap3.html#the_cross-entropy_cost_function)
...
![NikeExample](NikeExample.jpg "Example Nike Image")
...
Instead of better algorithms to explicitly define every permutation of logo that we would potentially see, machine learning and neural networks allow us to feed training images to a neural network and allow it to define for itself what the critical features are.
...
Convolutional neural networks have been shown to be effective at image classification.  ILSVR or ImageNet are both examples where CNNs achieve great performance and their [performance continues improving](http://bits.blogs.nytimes.com/2014/08/18/computer-eyesight-gets-a-lot-more-accurate/).
...
It is difficult to not anthropomorphize a computer algorithm that "sees" images.  One way to help us to understand how a CNN "viewing" an image is to look at what it would define as an ideal example of a particular class.  Examining different layers within our neural network, we can begin to appreciate how our network is building up features to identify objects.

![VGG16_filters](stitched_filters_8x8.png)

In this case, here is what VGG16 would say is "99.999% a shoe"

Using this tutorial to examine what neural networks see https://blog.keras.io/how-convolutional-neural-networks-see-the-world.html



[###todo visualize layers of model] otherwise not sure this is relevant
...
how does it do on the following?
...
![NikeCrop](NikeCropping.jpg "An Example of Image Cropping")

Adobe Lightroom was used to load, crop, and export images in both the Altra and Nike categories.
...
From there, the following steps will be performed:
...
##### Not just binary classification:
   Importantly I also wanted to differentiate between the brands of interest and other shoes/ brands.  I.E. I didn't want to categorize everything as either Nike or Altra, but rather be able to know that an image did not contain one of the logos I was tracking.
#### Additional images

#### Optimizer
  * [Gradient Descent](https://www.tensorflow.org/versions/r0.10/api_docs/python/train.html#GradientDescentOptimizer)
  * [Adaptive Gradient](http://www.jmlr.org/papers/volume12/duchi11a/duchi11a.pdf) or Adagrad has an implementation in [TensorFlow](https://www.tensorflow.org/versions/r0.10/api_docs/python/train.html#AdagradOptimizer)

#### Hyperparameter Search
  * Learning Rate [###todo add math symbol here]
  * Number of iterations
  * With/ Without 'Neither' category
  * Optimizer
    * [Gradient Descent](https://www.tensorflow.org/versions/r0.10/api_docs/python/train.html#GradientDescentOptimizer)
    * Adagrad most implementations start with an accumulator of [0.01]((http://sebastianruder.com/optimizing-gradient-descent/index.html#adagrad)) or [0.1](https://www.tensorflow.org/versions/r0.10/api_docs/python/train.html#AdagradOptimizer)
...
That only tells a portion of the story however.  Comparing the training and validation scores can be instructive

![TopTrainAccuracy](TopTrainAccuracy.png)
![TopValidationAccuracy](TopValidationAccuracy.png)

This shows us that although we get superior results from these at least two of the models (# 2 and 4) are saturating/ no longer learning, etc.
...
From the above information, and selecting for the model that uses multiclass classification, this is the ideal set of hyperparameters for our model.

|Top  Results|
|----|-----|------|------|------|------|------|
|  Run #  |Test Accuracy| Iterations |Learning Rate (η) |Data Set| Classification | Optimizer |
| 6       |   97.4%     |     8,000  |  0.001       |  First |

Here are the graphs of its performance compared to the previous top-runners.
[###todo graph of *the* best model]

With this finished model, we are able to compare against some difficult images from earlier:

[###todo difficult images from earlier, previous classification and current]

[###todo Other sensitivity analysis?]
...
[###todo simplified graphic of model]
...
[###todo insert images of torn up shoes and my models performance identifying them]
